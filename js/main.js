...
*/
  ...
  const newVideo = document.createElement("a-video")
  newVideo.setAttribute('width', '0.45')
  newVideo.setAttribute('height', '0.4')
  newVideo.setAttribute('position', '1 0.5 -1')
  newVideo.setAttribute('class', 'remote_video_holder')

  document.querySelector('#static_camera').appendChild(newVideo)
  ...
  remoteCount++
  video.setAttribute('id', 'newStream'+remoteCount);
  video.setAttribute('class', 'remote_video_asset')
  ...
  video.addEventListener('loadeddata', () => {
    ...
    newVideo.setAttribute('src', `#newStream`+remoteCount);
  });
...
export function hangupAction() {
  ...
  let rvhs = document.querySelectorAll('.remote_video_holder');
  for (var i in rvhs) {
    if (rvhs[i].parentNode) {
      rvhs[i].parentNode.removeChild(rvhs[i]);
    }
  }
  let rvas = document.querySelectorAll('.remote_video_asset');
  for (var i in rvas) {
    if (rvas[i].parentNode) {
      rvas[i].parentNode.removeChild(rvas[i]);
    }
  }
